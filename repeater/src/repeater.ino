...
void setup() {
  ...
  zbExtender.setManufacturerAndModel("ilmars-engineering", "extender");
  ...
  zigbeeConfig.nwk_cfg.zczr_cfg.max_children = 10;  // 10 is default
...
void loop() {
  ...
  if (!Zigbee.connected()) {
    ...
    if (lastAttempt && (millis() - lastAttempt > 30000)) {
      Serial.println("Not connected for more than 30s, restarting");
      ESP.restart();
    }
    delay(33);
    return;
